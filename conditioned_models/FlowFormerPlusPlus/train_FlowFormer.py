...
import core.datasetsA as datasets
...
def train(cfg):
    ...
    for name, param in model.named_parameters():
        ...
        if param.requires_grad:
            ...
            print(f"Does NOT require grad: {name}")
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            print(data_blob[0].shape)
            print(data_blob[2].shape)
            print(i_batch)
            if i_batch>50:
                break
            ...
            #req_flow = flow[0].permute(1, 2, 0)
            #req_flow = req_flow.cpu().numpy()
            #print("sdfd1")
            #print(req_flow.shape)
            #print("sdfd")
            #flow_img = flow_viz.flow_to_image(req_flow)
            #cv2.imwrite("to_del.png", flow_img[:, :, [2,1,0]])
            #exit(1)
