...
def parse_args():
    ...
    parser.add_argument('--mode', type=str,
                                choices=['uniform_blur', 'cfg', 'BB','BB_test', 'BB_w_blur', 'BB_w_blur2', 'BB_w_blur_extended', 'optical_flow','optical_flow_w_cfg'],
...
def basic_distillation(args):
    ...
    if quant:
        ...
        processor = AutoProcessor.from_pretrained(processor_id) if "BB" not in args.mode else SmolVLMProcessor.from_pretrained(processor_id)
        model = AutoModelForImageTextToText.from_pretrained(
            model_id,
    ...
    if "BB" in args.mode:
        ...
        if "extended" in args.mode:
            withExtended = True
            num_frames = 20 if "500M" in args.model_size else 10
