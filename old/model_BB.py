...
class SmolVLMCausalLMOutputWithPast(ModelOutput):
    ...
    bbox_predictions: Optional[torch.FloatTensor] = None
    bbox_confidence: Optional[torch.FloatTensor] = None
...
class SpatialAttentionPool(nn.Module):
    """
    Spatial attention pooling to extract spatial features from vision embeddings.
    """
    ...
    def forward(self, text_features, vision_features):
        ...
        Returns:
            attended_features: (batch_size, seq_len, text_dim)
        """
        # Project vision features to text dimension
        vision_proj = self.vision_proj(vision_features)  # (batch, num_patches, text_dim)
        ...
        attended_features = self.layer_norm(attended_features + text_features)
...
class BoundingBoxPredictor(nn.Module):
    """
    Predicts bounding boxes based on text embeddings and spatial attention.
    """
    def __init__(self, config, text_dim: int, vision_dim: int):
        super().__init__()
        self.config = config
        self.text_dim = text_dim
        self.vision_dim = vision_dim
        ...
        self.spatial_attention = SpatialAttentionPool(
    ...
    def _init_weights(self):
        ...
        for module in [self.bbox_head, self.confidence_head, self.text_aggregator]:
            for layer in module:
    ...
    def forward(self, text_embeddings, vision_features, attention_mask=None):
        """
        Args:
            text_embeddings: (batch_size, seq_len, text_dim)
            vision_features: (batch_size, num_patches, vision_dim)
            attention_mask: (batch_size, seq_len) - mask for text tokens
        Returns:
            bbox_predictions: (batch_size, 4) - normalized bounding box coordinates
        ...
        # Aggregate text features (mean pooling with attention mask)
        if attention_mask is not None:
            # Expand mask to match feature dimensions
            mask_expanded = attention_mask.unsqueeze(-1).expand_as(attended_features)
            # Apply mask and compute mean
            masked_features = attended_features * mask_expanded
            seq_lengths = attention_mask.sum(dim=1, keepdim=True).float()
            aggregated_features = masked_features.sum(dim=1) / seq_lengths
        else:
            aggregated_features = attended_features.mean(dim=1)
        
        # Process aggregated features
        processed_features = self.text_aggregator(aggregated_features)
        
        # Predict bounding boxes (normalized coordinates)
        bbox_predictions = self.bbox_head(processed_features)
        bbox_predictions = torch.sigmoid(bbox_predictions)  # Normalize to [0, 1]
        
        # Predict confidence scores
        confidence = self.confidence_head(processed_features).squeeze(-1)
        
        return bbox_predictions, confidence, attended_features
...
class SmolVLMForConditionalGeneration(SmolVLMPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.bbox_loss_weight = getattr(config, 'bbox_loss_weight', 1.0)
        self.confidence_loss_weight = getattr(config, 'confidence_loss_weight', 0.5)

        self.post_init()

    def compute_bbox_loss(self, pred_boxes, target_boxes, pred_confidence, target_confidence):
        """
        Compute bounding box regression loss.
        
        Args:
            pred_boxes: (batch_size, 4) - predicted boxes [x1, y1, x2, y2]
            target_boxes: (batch_size, 4) - target boxes [x1, y1, x2, y2]
            pred_confidence: (batch_size,) - predicted confidence scores
            target_confidence: (batch_size,) - target confidence (1.0 if box exists, 0.0 otherwise)
        """
        # Only compute bbox loss for samples with valid boxes
        valid_mask = target_confidence > 0.5
        
        if valid_mask.sum() == 0:
            # No valid boxes in batch
            bbox_loss = torch.tensor(0.0, device=pred_boxes.device, requires_grad=True)
        else:
    ...
    def forward(
        ...
        bbox_targets: Optional[torch.FloatTensor] = None,  # (batch_size, 4)
        bbox_confidence_targets: Optional[torch.FloatTensor] = None,  # (batch_size,)
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        cache_position: Optional[torch.LongTensor] = None,
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        #print(f"inside smolVLM cond {pixel_values.shape}") #the starting tokens
        #print(f"inside smolVLM cond {position_ids.shape}") #just positions ids for prompt
        ...
        if predict_bbox and outputs.image_hidden_states is not None:
            # Use the full hidden states for bbox prediction (not just the sliced ones)
            text_embeddings = hidden_states
            vision_features = outputs.image_hidden_states
            
            # Predict bounding boxes
            print("YES")
            bbox_predictions, bbox_confidence, attended_features = self.bbox_predictor(
        ...
        if bbox_targets is not None and bbox_confidence_targets is not None and predict_bbox:
            if bbox_predictions is not None and bbox_confidence is not None:
                bbox_loss, confidence_loss = self.compute_bbox_loss(
                    pred_boxes=bbox_predictions,
                    target_boxes=bbox_targets,
                    pred_confidence=bbox_confidence,
                    target_confidence=bbox_confidence_targets
                )
                
                total_loss += self.bbox_loss_weight * bbox_loss
                total_loss += self.confidence_loss_weight * confidence_loss
        ...
        if not return_dict:
            ...
            if bbox_predictions is not None:
                output = (bbox_predictions, bbox_confidence) + output
