...
class SmolVLMEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.depth == 26:
            import numpy as np
            l2 = torch.sum(hidden_states[18], dim=1)  # [729]
            ...
            heatmap_np = heatmap_up.squeeze().cpu().numpy()  # shape: [128, 128]
            ...
            # Save the image
            cv2.imwrite(f'to_del/a.png', heatmap_color)
            print("NAH")
...
class SmolVLMEncoder(nn.Module):
    ...
    def __init__(self, config: SmolVLMConfig):
        ...
        self.layers = nn.ModuleList([SmolVLMEncoderLayer(config, i) for i in range(config.num_hidden_layers)])
...
class SmolVLMConnector(nn.Module):
    def __init__(self, config):
        ...
        if "2.2B" in config._name_or_path:
            input_spatial_size = (27,27)
            output_spatial_size = 48
        else:
        ...
        #self.vis_to_flow = VisionToFlowTransform(input_dim=config.vision_config.hidden_size, input_spatial_size=input_spatial_size, output_spatial_size=output_spatial_size)
      
        self.vis_to_flow = VisionToFlowTransformGM(input_dim=config.vision_config.hidden_size, input_spatial_size=input_spatial_size, output_spatial_size=output_spatial_size)
    ...
    def forward(self, image_hidden_states,pixel_values):


        import numpy as np
        l2 = torch.sum(image_hidden_states[18], dim=1)  # [729]
        # Reshape to 27x27
        heatmap = l2.view(1, 1, 27, 27)  # [1, 1, H, W] for interpolate
        # Upsample to 128x128
        heatmap_up = F.interpolate(heatmap, size=(128, 128), mode='bilinear', align_corners=False)
        ...
        #print(f"BEFORE SHUFFLE: {image_hidden_states.shape}")
        #print(f"BEFORE SHUFFLE REQ: {image_hidden_states.requires_grad}")
        ...
        #        flow, _ = self.optical_flow(img1.float(), img2.float(),flow_states[i].float())
        #    else:
        #        flow = self.optical_flow(img1.float(), img2.float(),flow_states[i].float())
        #    
        #    flow = flow[0].permute(1, 2, 0).cpu().numpy()
        #    flow_img = flow_to_image(flow)
        #    cv2.imwrite(f"to_del/vis2_{i}.png", flow_img[:, :, [2,1,0]])

          

        #FOR FLOWFLORMER###################################
        #flow_states =  self.vis_to_flow(image_hidden_states.view(pixel_values.shape[0],pixel_values.shape[1],*image_hidden_states.shape[1:]))
        #optical_maps_pred = []
        #
        #for i in range(pixel_values.shape[1]-1):
        #    img1 = pixel_values[:,i,:,:,:]
...
class SmolVLMModel(SmolVLMPreTrainedModel):
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, pixel_attention_mask: torch.LongTensor = None):
        ...
        image_hidden_states, optical_maps_pred = self.connector(image_hidden_states,pixel_values_copy)
        return image_hidden_states, optical_maps_pred
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if pixel_values is not None and image_hidden_states is not None:
            ...
            image_hidden_states, optical_maps_pred = self.get_image_features(pixel_values, pixel_attention_mask)
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if (optical_flow_maps is not None) and (outputs.optical_maps_pred):
            ...
            print(f"optical maps labels {optical_flow_maps.shape}")
            print(f"length of optical predicted maps {len(outputs.optical_maps_pred)}")
            print(f"length elem in optical predicted maps {len(outputs.optical_maps_pred[0])}")
            print(f"length elemel in optical predicted maps {outputs.optical_maps_pred[0][0].shape}")
            ...
            for i in range(num_frames):
                ...
                x_optical_loss = self.sequence_loss(outputs.optical_maps_pred[i], optical_flow_maps[i])[0]
                optical_loss += x_optical_loss
        ...
        update_json(f"to_del/comp.json", d)
        print(optical_loss)
        exit(1)
