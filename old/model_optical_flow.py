...
from modules.optical_flow_decoder import OpticalFlowDecoder
...
class SmolVLMCausalLMOutputWithPast(ModelOutput):
    ...
    optical_flow: Optional[torch.FloatTensor] = None
    optical_flow_loss: Optional[torch.FloatTensor] = None
...
class SmolVLMForConditionalGeneration(SmolVLMPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.optical_flow_decoder = OpticalFlowDecoder()
        
        self.captioning_loss_weight   = getattr(config, 'captioning_loss_weight', 1.0)
        self.optical_flow_loss_weight = getattr(config, 'optical_flow_loss_weight', 0.1)
    ...
    def compute_optical_flow_loss(self, predicted_flow: torch.Tensor, 
    ...
    def forward(
        ...
        optical_flow_targets: Optional[torch.FloatTensor] = None,
        optical_flow_mask: Optional[torch.BoolTensor] = None,
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if outputs.image_hidden_states is not None:
            ...
            optical_flow = self.optical_flow_decoder(img_hidden)

            print(optical_flow.shape)
           
            
            # Compute optical flow loss if targets are provided
            if optical_flow_targets is not None:
                optical_flow_loss = self.compute_optical_flow_loss(
                    optical_flow, optical_flow_targets, optical_flow_mask
                )

        total_loss = None
        if captioning_loss is not None or optical_flow_loss is not None:
            total_loss = 0.0
            if captioning_loss is not None:
                total_loss += self.captioning_loss_weight * captioning_loss
            if optical_flow_loss is not None:
                total_loss += self.optical_flow_loss_weight * optical_flow_loss
    ...
    def prepare_inputs_for_generation(
        ...
        logits_to_keep=None,
        optical_flow_targets=None,
        optical_flow_mask=None,
        **kwargs,
    ):
        ...
        model_inputs['optical_flow_targets'] = optical_flow_targets
        model_inputs['optical_flow_mask'] = optical_flow_mask
