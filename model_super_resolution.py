...
from modules.super_resolution_decoder import SuperResolutionDecoder
...
class SmolVLMCausalLMOutputWithPast(ModelOutput):
    ...
    hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    attentions: Optional[Tuple[torch.FloatTensor]] = None
    captioning_loss: Optional[torch.FloatTensor] = None
    super_resolution: Optional[torch.FloatTensor] = None
    super_resolution_loss: Optional[torch.FloatTensor] = None
...
class SmolVLMForConditionalGeneration(SmolVLMPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.super_resolution_decoder = SuperResolutionDecoder()
        
        self.captioning_loss_weight   = getattr(config, 'captioning_loss_weight', 1.0)
        self.super_resolution_loss_weight = getattr(config, 'super_resolution_loss_weight', 0.1)
    ...
    def compute_super_resolution_loss(self, predicted_flow: torch.Tensor, 
                                  target_flow: torch.Tensor, 
                                  flow_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Compute optical flow loss using L1 + L2 loss.
        
        Args:
            predicted_flow: [batch_size, num_frames-1, 2, H, W]
            target_flow: [batch_size, num_frames-1, 2, H, W]
            flow_mask: [batch_size, num_frames-1, H, W] - optional mask for valid flow regions
        
        Returns:
            flow_loss: scalar tensor
        """
        # L1 loss (more robust to outliers)
        l1_loss = F.l1_loss(predicted_flow, target_flow, reduction='none')
        
        # L2 loss (smoother gradients)
        l2_loss = F.mse_loss(predicted_flow, target_flow, reduction='none')
        ...
        if flow_mask is not None:
            ...
            flow_mask = flow_mask.unsqueeze(2)  # [B, T-1, 1, H, W]
            total_loss = total_loss * flow_mask
            
            # Normalize by valid pixels
            valid_pixels = flow_mask.sum()
            if valid_pixels > 0:
                total_loss = total_loss.sum() / valid_pixels
            else:
    ...
    def forward(
        ...
        super_resolution_targets: Optional[torch.FloatTensor] = None,
        super_resolution_mask: Optional[torch.BoolTensor] = None,
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        print("INSIDE SMLVLM-CONDITIONAL")
        ...
        captioning_loss  = None
        if labels is not None:
            captioning_loss  = self.loss_function(
                logits=logits, labels=labels, vocab_size=self.config.text_config.vocab_size, **kwargs
            )
        

        super_resolution = None
        super_resolution_loss = None
        print(past_key_values.get_seq_length())
     
        #Use image_hidden_states from the vision encoder
        if outputs.image_hidden_states is not None:
            # Reshape image_hidden_states to [batch_size, num_frames, num_patches, hidden_dim]
            img_hidden = outputs.image_hidden_states
            if len(img_hidden.shape) == 3:  # [num_frame_sequence, 81, 2048]
                # Assume batch_size = 1 and add batch dimension
                img_hidden = img_hidden.unsqueeze(0)  # [1, num_frame_sequence, 81, 2048]
            
            # Generate optical flow
            super_resolution = self.super_resolution_decoder(img_hidden)

            print(super_resolution.shape)
           
            
            # Compute optical flow loss if targets are provided
            if super_resolution_targets is not None:
                super_resolution_loss = self.compute_super_resolution_loss(
                    super_resolution, super_resolution_targets, super_resolution_mask
                )

        total_loss = None
        if captioning_loss is not None or super_resolution_loss is not None:
            total_loss = 0.0
            if captioning_loss is not None:
                total_loss += self.captioning_loss_weight * captioning_loss
            if super_resolution_loss is not None:
                total_loss += self.super_resolution_loss_weight * super_resolution_loss
        ...
        return SmolVLMCausalLMOutputWithPast(
            ...
            super_resolution_loss=super_resolution_loss,
            logits=logits,
    ...
    def prepare_inputs_for_generation(
        ...
        logits_to_keep=None,
        super_resolution_targets=None,
        super_resolution_mask=None,
        **kwargs,
    ):
        ...
        model_inputs['super_resolution_targets'] = super_resolution_targets
        model_inputs['super_resolution_mask'] = super_resolution_mask
