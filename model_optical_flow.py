...
class TrajectoryEncoderLayer(nn.Module):
    def __init__(self, config: SmolVLMVisionConfig, i: 0):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.trackTention = TracktentionLayer(config.hidden_size)
    ...
    def forward(
        ...
        *prefix, P, D = hidden_states.shape
        sqrt_P = int(P**0.5)
...
class SmolVLMEncoder(nn.Module):
    ...
    def __init__(self, config: SmolVLMConfig):
        ...
        if config.use_cfg:
            self.trajectory_layers = nn.ModuleList([
                TrajectoryEncoderLayer(config, i) for i in range(self.num_trajectory_layers)
            ])
            self.trajectory_positions = [
                ((i + 1) * config.num_hidden_layers // (self.num_trajectory_layers + 1))
                for i in range(self.num_trajectory_layers)
            ]

            print(self.trajectory_positions)
        ...
        for layer_idx, encoder_layer in enumerate(self.layers):
            ...
            if (self.use_cfg and 
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    trajectory_outputs = self.trajectory_layers[trajectory_layer_idx](
                        hidden_states,
                        attention_mask,
                        output_attentions=output_attentions,
                ...
                hidden_states = trajectory_outputs[0]
                trajectory_layer_idx += 1
...
class SmolVLMVisionTransformer(SmolVLMPreTrainedModel):
    ...
    def forward(
        ...
        if self.use_optflow:
            ...
            for i in range(pixel_values_copy.shape[1]-1):
                img1 = pixel_values_copy[:,i,:,:,:]
                img2 = pixel_values_copy[:,i+1,:,:,:]


                if self.training == False:
                    #flow = self.optical_flow(img1.float(),  img2.float())['flow_preds']
                    flow = self.optical_flow(img1.float(),  flow_states[i].float())['flow_preds']
                else:
                    #flow = self.optical_flow(img1.float(),  img2.float())['flow_preds']
                    flow = self.optical_flow(img1.float(),  flow_states[i].float())['flow_preds']

                ######## 
                flowIMG = flow[-1][-1].permute(1, 2, 0).detach().cpu().numpy()
                flow_img = flow_to_image(flowIMG)
                cv2.imwrite(f"to_del/vis2_{i}TR.png", flow_img[:, :, [2,1,0]])
                ##########
                # 
                #  
...
class SmolVLMBaseModelOutputWithPast(ModelOutput):
    ...
    optical_maps_pred: Optional[List[torch.FloatTensor]] = None
...
class SmolVLMModel(SmolVLMPreTrainedModel):
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, pixel_attention_mask: torch.LongTensor = None,  pred_tracks:torch.LongTensor = None, pred_visibility:torch.BoolTensor= None):
        ...
        return image_hidden_states, optical_maps_pred
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        return SmolVLMBaseModelOutputWithPast(
            ...
            image_hidden_states=image_hidden_states,
            optical_maps_pred = optical_maps_pred
        )
...
class SmolVLMCausalLMOutputWithPast(ModelOutput):
    ...
    optical_loss: Optional[torch.FloatTensor] = None
...
class SmolVLMForConditionalGeneration(SmolVLMPreTrainedModel, GenerationMixin):
    ...
    def sequence_loss(self,flow_preds, flow_gt):
        ...
        valid =  (mag < max_flow) & (valid_gt >= 0.5)
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        #    for i in range(pixel_values.shape[1]-1):
        #        img1 = pixel_values[:,i,:,:,:]
        #        img2 = pixel_values[:,i+1,:,:,:]
        #        if self.training == False:
        #            flow, _ = self.optical_flow(img1.float(), img2.float())
        #        else:
        #            flow = self.optical_flow(img1.float(), img2.float())



                #flow = flow[0].permute(1, 2, 0).cpu().numpy()
                #flow_img = flow_to_image(flow)
                #cv2.imwrite(f"to_del/vis_{i}.png", flow_img[:, :, [2,1,0]])
        #else:
        #    pass
        ...
        if (optical_flow_maps is not None) and (outputs.optical_maps_pred):
            ...
            num_frames = optical_flow_maps.shape[0]
            optical_loss = 0.
            ...
            for i in range(num_frames):
                ...
                ##print((outputs.optical_maps_pred[i][0]).requires_grad)
                ##print(optical_flow_maps[i].requires_grad)
                #optical_flow_mapsIMG = optical_flow_maps[i][0].permute(1, 2, 0).detach().cpu().numpy()
                #flow_img = flow_to_image(optical_flow_mapsIMG)
                #cv2.imwrite(f"to_del/vis2_{i}GT.png", flow_img[:, :, [2,1,0]])

                
                #print(outputs.optical_maps_pred[i][-1].max())
                #print(optical_flow_maps[i].max())
                #print("\n")

                #print(outputs.optical_maps_pred[i][-1].min())
                #print(optical_flow_maps[i].min())
                #print("-----------------------")


                
                optical_loss += self.sequence_loss(outputs.optical_maps_pred[i], optical_flow_maps[i])[0]

                #d[i] = {}
                #d[i]["mine"] = x_optical_loss.item()
            optical_loss = optical_loss / num_frames
        else:
            pass  
        ...
        elif (cap_loss is not None) and (optical_loss is not None):
            loss = optical_loss + cap_loss
        else:
            loss = None
