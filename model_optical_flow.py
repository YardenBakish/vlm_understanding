...
class TrajectoryEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        *prefix, P, D = hidden_states.shape
        ...
        hidden_states =  self.trackTention(hidden_states.view(*new_shape),pred_tracks.squeeze(0))
...
class SmolVLMEncoder(nn.Module):
    ...
    def __init__(self, config: SmolVLMConfig):
        ...
        layers_lst = []
        positions = [
        ((i + 1) * (config.num_hidden_layers + i) // (self.num_trajectory_layers + 1))
        ...
        if config.use_cfg:
            for i, pos in enumerate(positions):
                ...
                layers_lst.insert(pos, TrajectoryEncoderLayer(config, i))
...
class SmolVLMModel(SmolVLMPreTrainedModel):
    ...
    def __init__(self, config: SmolVLMConfig, use_cfg: False):
        super().__init__(config)
...
class SmolVLMForConditionalGeneration(SmolVLMPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, use_cfg=False):
        super().__init__(config)
        self.model = SmolVLMModel(config, use_cfg=use_cfg)
        self.image_token_id = self.config.image_token_id
