...
from transformers.utils import is_num2words_available, logging
        ...
        **kwargs,
        ...
        def sample_indices_fn_func(metadata, **fn_kwargs):
            return smolvlm_sample_indices_fn(
                metadata, max_frames=max_frames, target_fps=target_fps, skip_secs=skip_secs,selected_index=selected_index, extended=extended,num_frames=num_frames, **fn_kwargs
            )
        ...
        conversations = super().apply_chat_template(
            conversation, video_load_backend=video_load_backend, sample_indices_fn=sample_indices_fn, **kwargs
        )
       
        if return_frame_indices:
            indices_samples = sample_indices_fn_func(self.last_metadata)
            return conversations, indices_samples
        else:
            return conversations
