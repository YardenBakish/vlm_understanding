...
class FlowFormer(nn.Module):
    ...
    def forward(self, image1, image2, cost_memory, mask=None, output=None, flow_init=None):
        ...
        if False:
            ...
            T, cost_patches, feat_s_quater, feat_t_quater = self.memory_encoder(image1, image2, data, context)

            #print(f"cost_memory: {T.shape}")
            #print(f"cost_memory: {cost_memory.shape}")
            #exit(1)

           
            
            #print(f"COST MAPS: {data['cost_maps'].shape}")

            #print(cost_patches.shape)
             #NONE feat_s_quater
             # NONE feat_t_quater
             #NONE   flow_init.shape
            #print(cost_patches.shape)
            #print(data['cost_maps'].shape)
        
          
            flow_predictions = self.memory_decoder(cost_memory, context, None, None, None, data, flow_init=None, cost_patches=None)
