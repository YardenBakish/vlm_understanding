...
# you may not use this file exscept in compliance with the License.
...
class MovementEncoder(nn.Module):
    """
    Simple learnable encoder for 2D movement vectors.
    Takes discrete values in range [-512, 512] and embeds to D dimensions.
    """
    
    def __init__(
        self, 
        embed_dim: int = 1152,
        fourier_dim: int = 512,  # Much smaller
        ...
        # Simple positional encoding frequencies
        # For discrete values [-512, 512], we don't need many frequencies
        num_freqs = fourier_dim // 4  # div by 4 since we do sin/cos for x and y
        
        # Simple linear spacing of frequencies
        freq_bands = torch.linspace(0.1, math.pi, num_freqs)
    ...
    def forward(self, movement_vectors: torch.Tensor) -> torch.Tensor:
        """
        Args:
            movement_vectors: (TB, PP, 2), discrete pixel offsets (dx, dy) in [-512, 512]
        Returns:
            encoded_movement: (TB, PP, embed_dim)
        """
        # Simple normalization to [-1, 1] range
        normalized = movement_vectors / 512.0
        
        # Split dx, dy
        dx = normalized[..., 0:1]  # (TB, PP, 1)
        dy = normalized[..., 1:2]  # (TB, PP, 1)
        
        # Apply frequency encoding
        dx_freqs = dx * self.freq_bands  # (TB, PP, num_freqs)
        dy_freqs = dy * self.freq_bands  # (TB, PP, num_freqs)
        ...
        fourier_feats = torch.cat([
            ...
            torch.sin(dy_freqs), 
            torch.cos(dy_freqs)
        ], dim=-1)  # (TB, PP, fourier_dim)
        
        # Project to target dimension
        encoded = self.mlp(fourier_feats)
...
class SmolVLMVisionTransformer(SmolVLMPreTrainedModel):
    ...
    def __init__(self, config: SmolVLMVisionConfig):
        ...
        self.movement_encoder  = MovementEncoder(
            embed_dim=config.hidden_size,
    ...
    def forward(
        ...
        movement_embedding = self.movement_encoder(movement_vectors)
        T = 30
        B = last_hidden_state.shape[0] // T
        H, D = movement_embedding.shape[1:]  # 729, 1152

        # reshape to [B, T-1, H, D]
        movement_embedding_reshaped = movement_embedding.view(B, T-1, H, D)

        # create a zero tensor for padding
        zero_frame = torch.zeros(B, 1, H, D, device=movement_embedding.device, dtype=movement_embedding.dtype)

        # concatenate zeros at the end along the temporal dimension
        movement_embedding_padded = torch.cat([movement_embedding_reshaped, zero_frame], dim=1)

        # reshape back to [B*T, H, D]
        movement_embedding_padded = movement_embedding_padded.view(B*T, H, D)


        last_hidden_state = last_hidden_state + movement_embedding_padded
...
class SmolVLMModel(SmolVLMPreTrainedModel):
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, pixel_attention_mask: torch.LongTensor = None, movement_vectors:torch.LongTensor = None,):
        ...
        image_hidden_states = self.vision_model(pixel_values=pixel_values, patch_attention_mask=patch_attention_mask, movement_vectors= movement_vectors,)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if pixel_values is not None and image_hidden_states is not None:
            ...
            image_hidden_states = self.get_image_features(pixel_values, pixel_attention_mask,movement_vectors)
